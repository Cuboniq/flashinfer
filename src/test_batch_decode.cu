...
void _TestBatchDecodingKernelCorrectness(size_t page_size, size_t batch_size, size_t num_qo_heads,
  ...
  if (!cooperative) {
    ...
    cudaError_t status =
        flashinfer::BatchDecodeWithPagedKVCache<PageStorage::kIndices, T, T, int32_t>(
    ...
    cudaError_t status =
        flashinfer::BatchDecodeWithPagedKVCacheWrapper<PageStorage::kIndices, T, T, int32_t>(
            &handler, thrust::raw_pointer_cast(q_device.data()), /*q_rope_position=*/nullptr,
            paged_kv, thrust::raw_pointer_cast(o_device.data()), /*lse=*/nullptr, num_qo_heads,
            rotary_mode);
    EXPECT_EQ(status, cudaSuccess) << "CUDA error: " + std::string(cudaGetErrorString(status));
  }
  // compare result
